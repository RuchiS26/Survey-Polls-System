...
def spolen_index(request):
	...
	entpoll = 'poll'
	entsurvey = 'survey'
	return render(request, "spolen/spolen-index.html", {'polls':polls,'surveys':surveys,'entpoll':entpoll,'entsurvey':entsurvey})
...
def spolen_polls_participate(request,slug):
	...
	poll = Poll.objects.get(slug = slug)
	if( PollAccess.objects.filter(poll = poll ).filter(user = request.user).exists() or poll.user == request.user):
		messages.info(request,f"Poll already answered or is created by you!")
...
def spolen_surveys_participate(request,slug):
	...
	if( SurveyAccess.objects.filter(survey = survey ).filter(user = request.user).exists() or survey.user == request.user):
		messages.info(request,f"Survey already answered or is created by you!")
		return redirect('spolen:spolen-index')
