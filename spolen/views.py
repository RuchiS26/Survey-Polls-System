...
def spolen_index(request):
	...
	if request.user.is_superuser:
		polls_cl = Poll.objects.filter(closed = True).order_by('-date_posted')
	else:
	...
	if request.user.is_superuser:
		surveys_cl = Survey.objects.filter(closed = True).order_by('-date_posted')
	else:
...
def spolen_create_poll(request):
	if request.method == 'POST':
		...
		if(pollform.is_valid()):
			...
			poll_count = Poll.objects.filter(user = request.user).count() + 1
			now = datetime.datetime.now().time()
			slug = str(request.user)+'poll-'+str(poll_count) + "-" + str(now)
			instance.slug = slug
...
def spolen_create_survey(request):
	...
	if request.method == 'POST':
		...
		if(surveyform.is_valid()):
			...
			survey_count = Survey.objects.filter(user = request.user).count() + 1
			now = datetime.datetime.now().time()
			slug = str(request.user)+'survey-'+str(survey_count) + "-" + str(now)
			instance.slug = slug
...
def spolen_discard(request,ent,slug):
	...
	if(ent == 'poll'):
		ent = Poll.objects.get(slug = slug)
		if(ent.user == request.user or request.user.is_superuser):

			ent.delete()
			messages.success(request,f'Poll discarded!')

		else:
			messages.success(request,f'Invalid Access!')
		...
		if(ent.user == request.user or request.user.is_superuser):
			...
			ent.delete()
			messages.success(request,f'Survey discarded!')
...
def spolen_analytics(request,ent,slug):
	...
	if(ent == 'poll'):
		...
		if(poll.user == request.user or request.user.is_superuser):
			...
			for option in options:
				res_count = res_count + option.votes

			return render(request, 'spolen/spolen-analytics.html', {'poll':poll,'options':options,'ent':ent, 'res_count':res_count})
		...
		if(survey.user == request.user or request.user.is_superuser):
			...
			return render(request, 'spolen/spolen-analytics.html', {'survey':survey,'saDict':saDict,'mcqDict':mcqDict,'ent':ent,'questions':questions,'res_count':res_count})
